...
func (b *Builder) Build() (*Service, error) {
	...
	// Initialize MongoDB store and merge database configurations into the loaded config.
	// When MongoDB is enabled, database configurations take precedence over yml file settings.
	var mongoStoreInst *store.MongoStore
	if b.cfg != nil {
		log.Infof("cliproxy: MongoDB config - enabled=%v, uri=%s", b.cfg.MongoDB.Enabled, b.cfg.MongoDB.URI)
		if b.cfg.MongoDB.Enabled {
			var mongoErr error
			mongoStoreInst, mongoErr = store.NewMongoStore(b.cfg.MongoDB)
			if mongoErr != nil {
				log.Errorf("cliproxy: failed to initialize mongodb store: %v", mongoErr)
			} else if mongoStoreInst != nil {
				ctxMongo, cancelMongo := context.WithTimeout(context.Background(), 30*time.Second)
				mongoCfg, errConvert := mongoStoreInst.ConvertToConfig()
				if errConvert != nil {
					log.Errorf("cliproxy: failed to convert mongodb configs: %v", errConvert)
				} else if mongoCfg != nil {
					if len(mongoCfg.GeminiKey) > 0 {
						b.cfg.GeminiKey = append(b.cfg.GeminiKey, mongoCfg.GeminiKey...)
						b.cfg.SanitizeGeminiKeys()
					}
					if len(mongoCfg.CodexKey) > 0 {
						b.cfg.CodexKey = append(b.cfg.CodexKey, mongoCfg.CodexKey...)
						b.cfg.SanitizeCodexKeys()
					}
					if len(mongoCfg.ClaudeKey) > 0 {
						b.cfg.ClaudeKey = append(b.cfg.ClaudeKey, mongoCfg.ClaudeKey...)
						b.cfg.SanitizeClaudeKeys()
					}
					if len(mongoCfg.OpenAICompatibility) > 0 {
						b.cfg.OpenAICompatibility = append(b.cfg.OpenAICompatibility, mongoCfg.OpenAICompatibility...)
						b.cfg.SanitizeOpenAICompatibility()
						log.Infof("cliproxy: merged %d openai-compatibility configs from mongodb", len(mongoCfg.OpenAICompatibility))
					}
					log.Infof("cliproxy: merged configs from mongodb - %d gemini keys, %d codex keys, %d claude keys",
						len(mongoCfg.GeminiKey), len(mongoCfg.CodexKey), len(mongoCfg.ClaudeKey))
				}
			if errRegister := mongoStoreInst.RegisterModelsFromConfig(ctxMongo); errRegister != nil {
				log.Errorf("cliproxy: failed to register models from mongodb: %v", errRegister)
			}
			cancelMongo()
		}
		}
