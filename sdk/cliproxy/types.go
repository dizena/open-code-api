...
type WatcherWrapper struct {
	...
	applyConfigReload     func(cfg *config.Config)
...
// ApplyRuntimeConfigReload updates the watcher config cache and triggers the same
// runtime auth refresh path used for filesystem-driven config changes.
func (w *WatcherWrapper) ApplyRuntimeConfigReload(cfg *config.Config) {
	if w == nil || w.applyConfigReload == nil {
		return
	}
	w.applyConfigReload(cfg)
}
