...
func (m *Manager) resolveOpenAICompatUpstreamModelPool(auth *Auth, requestedModel string) []string {
	if m == nil || !isOpenAICompatAuth(auth) {
		return nil
	}
	requestedModel = strings.TrimSpace(requestedModel)
	if requestedModel == "" {
...
func (m *Manager) rebuildAPIKeyModelAliasLocked(cfg *internalconfig.Config) {
	...
	for _, auth := range m.auths {
		...
		isAPIKey := isAPIKeyAuth(auth)
		isCompat := isOpenAICompatAuth(auth)
		if !isAPIKey && !isCompat {
			continue
		}
		...
		default:
			...
			if isCompat || compatName != "" || strings.EqualFold(strings.TrimSpace(auth.Provider), "openai-compatibility") {
...
func (m *Manager) applyAPIKeyModelAlias(auth *Auth, requestedModel string) string {
	...
	if isCompat && !isAPIKey {
		if upstreamModel := resolveUpstreamModelForOpenAICompatAPIKey(cfg, auth, requestedModel); upstreamModel != "" {
