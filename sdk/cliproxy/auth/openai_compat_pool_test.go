...
func newOpenAICompatHeaderAuthTestManager(t *testing.T, alias string, models []internalconfig.OpenAICompatibilityModel, executor *openAICompatPoolExecutor) *Manager {
	t.Helper()
	cfg := &internalconfig.Config{
		OpenAICompatibility: []internalconfig.OpenAICompatibility{{
			Name:   "pool",
...
func TestManagerExecute_OpenAICompatAliasResolvesWithoutAPIKey(t *testing.T) {
	alias := "geo-pro"
	executor := &openAICompatPoolExecutor{id: "pool"}
	m := newOpenAICompatHeaderAuthTestManager(t, alias, []internalconfig.OpenAICompatibilityModel{
		{Name: "MiniMax-M2.5", Alias: alias},
	}, executor)
	...
	if string(resp.Payload) != "MiniMax-M2.5" {
		t.Fatalf("payload = %q, want %q", string(resp.Payload), "MiniMax-M2.5")
	}
	got := executor.ExecuteModels()
	if len(got) != 1 || got[0] != "MiniMax-M2.5" {
		t.Fatalf("execute calls = %v, want [%q]", got, "MiniMax-M2.5")
	}
}

func TestManagerExecute_OpenAICompatAliasPoolRotatesWithoutAPIKey(t *testing.T) {
	alias := "geo-pro"
	executor := &openAICompatPoolExecutor{id: "pool"}
	m := newOpenAICompatHeaderAuthTestManager(t, alias, []internalconfig.OpenAICompatibilityModel{
		{Name: "MiniMax-M2.5", Alias: alias},
		{Name: "qwen-max", Alias: alias},
	}, executor)
	...
	got := executor.ExecuteModels()
	want := []string{"MiniMax-M2.5", "qwen-max", "MiniMax-M2.5"}
	if len(got) != len(want) {
		t.Fatalf("execute calls = %v, want %v", got, want)
	}
	for i := range want {
