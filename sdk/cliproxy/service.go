...
type Service struct {
	...
	// mongoUsageStore stores usage records to MongoDB when home is disabled.
	mongoUsageStore *store.MongoUsageStore
}
...
func mergeMongoRuntimeConfig(baseCfg *config.Config, mongoCfg *config.Config) {
	if baseCfg == nil || mongoCfg == nil {
		return
	}
	if len(mongoCfg.GeminiKey) > 0 {
		baseCfg.GeminiKey = append(baseCfg.GeminiKey, mongoCfg.GeminiKey...)
		baseCfg.SanitizeGeminiKeys()
	}
	if len(mongoCfg.VertexCompatAPIKey) > 0 {
		baseCfg.VertexCompatAPIKey = append(baseCfg.VertexCompatAPIKey, mongoCfg.VertexCompatAPIKey...)
		baseCfg.SanitizeVertexCompatKeys()
	}
	if len(mongoCfg.CodexKey) > 0 {
		baseCfg.CodexKey = append(baseCfg.CodexKey, mongoCfg.CodexKey...)
		baseCfg.SanitizeCodexKeys()
	}
	if len(mongoCfg.ClaudeKey) > 0 {
		baseCfg.ClaudeKey = append(baseCfg.ClaudeKey, mongoCfg.ClaudeKey...)
		baseCfg.SanitizeClaudeKeys()
	}
	if len(mongoCfg.OpenAICompatibility) > 0 {
		baseCfg.OpenAICompatibility = append(baseCfg.OpenAICompatibility, mongoCfg.OpenAICompatibility...)
		baseCfg.SanitizeOpenAICompatibility()
	}
}

func (s *Service) reloadModelConfigFromMongo(ctx context.Context) error {
	if s == nil {
	...
	baseCfg, err := config.LoadConfigOptional(s.configPath, false)
	if err != nil {
		return fmt.Errorf("reload base config: %w", err)
	}
	if baseCfg == nil {
		baseCfg = &config.Config{}
	}
	mongoCfg, err := mongoStore.ConvertToConfig()
	if err != nil {
		return fmt.Errorf("load mongodb model config: %w", err)
	}
	mergeMongoRuntimeConfig(baseCfg, mongoCfg)
	if ctx == nil {
		ctx = context.Background()
	}
	if err := mongoStore.RegisterModelsFromConfig(ctx); err != nil {
		return fmt.Errorf("sync mongodb model registry: %w", err)
	}
	if s.watcher != nil {
		s.watcher.ApplyRuntimeConfigReload(baseCfg)
		return nil
	}
	s.applyConfigUpdate(baseCfg)
	return nil
}
...
func (s *Service) Run(ctx context.Context) error {
	...
	if homeEnabled {
		...
		// When home is disabled, store usage records to MongoDB.
		mongoStore, err := store.NewMongoUsageStore(store.GetMongoStore())
		if err != nil {
			return fmt.Errorf("failed to initialize mongo usage store: %w", err)
		}
		if mongoStore != nil {
			s.mongoUsageStore = mongoStore
			usage.RegisterPlugin(mongoStore)
		}
	}
	...
	serverOptions = append(serverOptions, api.WithAdminModelRefreshHook(func(ctx context.Context) error {
		return s.reloadModelConfigFromMongo(ctx)
	}))
...
func (s *Service) Shutdown(ctx context.Context) error {
	...
	s.shutdownOnce.Do(func() {
		...
		if s.mongoUsageStore != nil {
			if err := s.mongoUsageStore.Close(); err != nil {
				log.WithError(err).Warn("failed to close mongo usage store")
			}
			s.mongoUsageStore = nil
		}
