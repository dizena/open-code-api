...
func TestOpenAICompatExecutorRewritesPayloadModelFromResolvedRequestModel(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
	...
	payload := []byte(`{"model":"geo-pro","messages":[{"role":"user","content":"hi"}]}`)
	_, err := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		Model:   "MiniMax-M2.5",
		Payload: payload,
	}, cliproxyexecutor.Options{
	...
	if got := gjson.GetBytes(gotBody, "model").String(); got != "MiniMax-M2.5" {
		t.Fatalf("model = %q, want %q; body=%s", got, "MiniMax-M2.5", string(gotBody))
	}
}

func TestOpenAICompatExecutorRewritesResponsesPayloadModelFromResolvedRequestModel(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"id":"chatcmpl_1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"2"},"finish_reason":"stop"}],"usage":{"prompt_tokens":1,"completion_tokens":1,"total_tokens":2}}`))
	}))
	defer server.Close()
	...
	payload := []byte(`{"model":"geo-pro","instructions":"You are a helpful assistant.","input":[{"role":"user","content":[{"type":"input_text","text":"1+1=?"}]}],"stream":false}`)
	_, err := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		Model:   "MiniMax-M2.5",
		Payload: payload,
	}, cliproxyexecutor.Options{
	...
	if !gjson.GetBytes(gotBody, "messages").Exists() {
		t.Fatalf("expected responses payload to be translated to chat completions; body=%s", string(gotBody))
	}
}
