...
type Config struct {
	...
	// MongoDB config for model configurations stored in database.
	MongoDB MongoConfig `yaml:"mongodb" json:"mongodb"`

	// SMTP config for email verification code sending.
...
// MongoConfig configures MongoDB connection for model configurations.
type MongoConfig struct {
	// Enabled toggles the MongoDB-backed model configuration.
	Enabled bool `yaml:"enabled" json:"enabled"`
	// URI is the MongoDB connection string.
	URI string `yaml:"uri" json:"uri"`
	// Database overrides the database name extracted from the URI. Optional.
	Database string `yaml:"database" json:"database"`
}

// GetDatabase returns the database name: explicit field > URI path > default "proxy".
func (m MongoConfig) GetDatabase() string {
	if m.Database != "" {
		return m.Database
	}
	...
	if idx := strings.Index(uri, "://"); idx != -1 {
		afterProto = uri[idx+3:]
	}
	if idx := strings.Index(afterProto, "/"); idx != -1 {
		dbPart := afterProto[idx+1:]
		if qIdx := strings.IndexAny(dbPart, "?&"); qIdx != -1 {
			dbPart = dbPart[:qIdx]
		}
		if dbPart != "" {
			return dbPart
...
// SMTPConfig configures SMTP email service for sending verification codes.
type SMTPConfig struct {
	// Enabled toggles the SMTP email service.
	Enabled bool `yaml:"enabled" json:"enabled"`
