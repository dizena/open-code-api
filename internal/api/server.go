...
import (
	...
	"github.com/router-for-me/CLIProxyAPI/v7/internal/api/handlers/userapi"
...
type serverOptionConfig struct {
	...
	adminModelRefresh    func(context.Context) error
...
// WithAdminModelRefreshHook registers a hook that refreshes runtime model/auth state
// after admin model CRUD operations succeed.
func WithAdminModelRefreshHook(refresh func(context.Context) error) ServerOption {
	return func(cfg *serverOptionConfig) {
		cfg.adminModelRefresh = refresh
	}
}
...
type Server struct {
	...
	// baseListener is the shared TCP listener.
	...
	// user API handlers and JWT secret
	jwtSecret     string
	authHandler   *userapi.AuthHandler
	userHandler   *userapi.UserHandler
	adminHandler  *userapi.AdminHandler
	jwtMiddleware *userapi.JWTMiddleware
}
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	// Initialize JWT secret and user API handlers
	s.jwtSecret = cfg.JWTSecret
	if s.jwtSecret == "" {
		s.jwtSecret = "cli-proxy-api-default-jwt-secret-change-me"
	}
	mongoStore := store.GetMongoStore()
	if mongoStore != nil && mongoStore.IsEnabled() {
		mailSvc := mail.NewService(cfg.SMTP)
		s.authHandler = userapi.NewAuthHandler(mongoStore, s.jwtSecret, mailSvc)
		s.userHandler = userapi.NewUserHandler(mongoStore)
		s.adminHandler = userapi.NewAdminHandler(mongoStore, optionState.adminModelRefresh)
		s.jwtMiddleware = userapi.NewJWTMiddleware(s.jwtSecret, mongoStore)
	}
...
func (s *Server) setupRoutes() {
	...
	v1.Use(s.apiKeyAuthMiddleware(), middleware.BalanceCheckMiddleware())
	{
	...
	codexDirect.Use(s.apiKeyAuthMiddleware(), middleware.BalanceCheckMiddleware())
	{
	...
	v1beta.Use(s.apiKeyAuthMiddleware(), middleware.BalanceCheckMiddleware())
	{
	...
	s.engine.GET("/anthropic/callback", func(c *gin.Context) {
		...
		if state != "" {
			_, _ = writeOAuthCallbackFileForPendingSession(s.cfg.AuthDir, "anthropic", state, code, errStr)
		}
	...
	s.engine.GET("/codex/callback", func(c *gin.Context) {
		...
		if state != "" {
			_, _ = writeOAuthCallbackFileForPendingSession(s.cfg.AuthDir, "codex", state, code, errStr)
		}
		c.Header("Content-Type", "text/html; charset=utf-8")
		c.String(http.StatusOK, oauthCallbackSuccessHTML)
	})
	...
	s.engine.GET("/google/callback", func(c *gin.Context) {
		...
		if state != "" {
			_, _ = writeOAuthCallbackFileForPendingSession(s.cfg.AuthDir, "gemini", state, code, errStr)
		}
		c.Header("Content-Type", "text/html; charset=utf-8")
		c.String(http.StatusOK, oauthCallbackSuccessHTML)
	})
	...
	s.engine.GET("/antigravity/callback", func(c *gin.Context) {
		...
		if state != "" {
			_, _ = writeOAuthCallbackFileForPendingSession(s.cfg.AuthDir, "antigravity", state, code, errStr)
		}
		c.Header("Content-Type", "text/html; charset=utf-8")
		c.String(http.StatusOK, oauthCallbackSuccessHTML)
	})
	...
	s.engine.GET("/xai/callback", func(c *gin.Context) {
		...
		if state != "" {
			_, _ = writeOAuthCallbackFileForPendingSession(s.cfg.AuthDir, "xai", state, code, errStr)
		}
		c.Header("Content-Type", "text/html; charset=utf-8")
		c.String(http.StatusOK, oauthCallbackSuccessHTML)
	})
...
func (s *Server) setupUserRoutes() {
	if s.authHandler == nil || s.jwtMiddleware == nil {
		return
	}

	// Auth routes (no auth required)
	s.engine.POST("/v0/auth/login", s.authHandler.Login)
	s.engine.POST("/v0/auth/send-code", s.authHandler.SendCode)
	s.engine.POST("/v0/auth/verify-code", s.authHandler.VerifyCode)
	s.engine.GET("/v0/auth/check-account", s.authHandler.CheckAccount)

	// User routes (JWT auth required)
	user := s.engine.Group("/v0/user")
	user.Use(s.jwtMiddleware.Handle())
	{
		user.GET("/keys", s.userHandler.ListKeys)
		user.POST("/keys", s.userHandler.CreateKey)
		user.DELETE("/keys/:key", s.userHandler.DeleteKey)
		user.GET("/usage", s.userHandler.ListUsage)
		user.GET("/models", s.userHandler.ListAvailableModels)
		user.GET("/balance", s.userHandler.GetBalance)
		user.POST("/change-password", s.userHandler.ChangePassword)
	}

	// Admin routes (JWT auth + admin role required)
	admin := s.engine.Group("/v0/admin")
	admin.Use(s.jwtMiddleware.Handle(), s.jwtMiddleware.RequireAdmin())
	{
		// User management
		admin.GET("/users", s.adminHandler.ListUsers)
		admin.POST("/users", s.adminHandler.CreateUser)
		admin.PUT("/users/:account", s.adminHandler.UpdateUser)
		admin.DELETE("/users/:account", s.adminHandler.DeleteUser)

		// Model management
		admin.GET("/models", s.adminHandler.ListModels)
		admin.POST("/models", s.adminHandler.CreateModel)
		admin.PUT("/models/:id", s.adminHandler.UpdateModel)
		admin.DELETE("/models/:id", s.adminHandler.DeleteModel)
		// Alias management
		admin.GET("/aliases", s.adminHandler.ListAliases)
		admin.PUT("/models/:id/alias", s.adminHandler.UpdateModelAlias)


		admin.GET("/keys", s.adminHandler.ListUserKeys)
		admin.POST("/keys", s.adminHandler.CreateKeyForUser)
		admin.DELETE("/keys/:key", s.adminHandler.DeleteUserKey)
		admin.PUT("/keys/:key/disable", s.adminHandler.DisableUserKey)

		// Usage query
		admin.GET("/usage", s.adminHandler.ListUsage)
	}
}
...
func (s *Server) Start() error {
	...
	case errServe := <-httpErrCh:
		if s.baseListener != nil {
			if errClose := s.baseListener.Close(); errClose != nil && !errors.Is(errClose, net.ErrClosed) {
...
// apiKeyAuthMiddleware authenticates requests by validating the API key against t_user_key in MongoDB.
// It replaces the previous access manager-based authentication.
func (s *Server) apiKeyAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		mongoStore := store.GetMongoStore()
		if mongoStore == nil || !mongoStore.IsEnabled() {
			// MongoDB not available: allow request through for backward compatibility.
			// In production, MongoDB should be configured for proper authentication.
			c.Set("userApiKey", apiKey)
			c.Next()
			return
		}

		userKey, err := mongoStore.GetUserKeyByKey(c.Request.Context(), apiKey)
		if err != nil {
			log.WithError(err).Error("apiKeyAuthMiddleware: lookup key error")
		...
		if userKey.Status == store.UserKeyDisabled {
		...
		go func() {
			...
			defer cancel()
			_ = mongoStore.TouchUserKeyLastUsed(ctx, apiKey)
		}()

		c.Set("userApiKey", apiKey)
		c.Set("userId", userKey.UserID.Hex())
		c.Next()
	}
}
