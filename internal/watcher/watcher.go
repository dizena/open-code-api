...
// ApplyRuntimeConfigReload updates the cached config, triggers the server reload callback,
// and rebuilds synthesized auth state without requiring a filesystem event.
func (w *Watcher) ApplyRuntimeConfigReload(cfg *config.Config) {
	if w == nil || cfg == nil {
		return
	}
	w.SetConfig(cfg)
